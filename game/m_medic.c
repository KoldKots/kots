...
edict_t *medic_FindDeadMonster (edict_t *self)
{
    ...
    while ((ent = findradius(ent, self->s.origin, 1024)) != NULL)
    {
        ...
        //SWB - since everything goes in the body queue now we need to make sure that it's in the body queue
        //and it has a targetname in order to be resurrected
        if (Q_stricmp(ent->classname, "bodyque") != 0 || !ent->targetname)
            continue;
...
{
    //SWB - Need an attack while ducking
    //if (random() > 0.25)
        return;
...
void medic_cable_attack (edict_t *self)
{
    ...
    //SWB - ensure that we have an enemy at this point
    //It's possible that we don't because we couldn't find a spawn function
    ...
    {
        //SWB - increase the monster count due to resurrection
        //dont' add here because monster_start will increment it
        //if (!coop->value)
            //level.total_monsters++;

        //ensure bodies get taken out of the bodyque correctly
        if (Q_stricmp(self->enemy->classname, "bodyque") == 0)
        {
            if (self->enemy->targetname)
            {
                edict_t *ent = G_Spawn();
                ent->classname = self->enemy->targetname;
                VectorCopy(self->enemy->s.origin, ent->s.origin);
                VectorCopy(self->enemy->s.origin, ent->s.old_origin);
                ...
                if (self->enemy->die)
                {
                    ...
                    //attempt to gib it
                    self->enemy->die(self->enemy, self->enemy, self->enemy, 0, self->enemy->s.origin);
                }

                self->enemy = ent;
                gi.linkentity(ent);
            }
            else //just gib the body and return
            {
                if (self->enemy->die)
                {
                    //ensure that we can gib any corpse
                ...
                self->enemy = NULL;
                gi.dprintf("ERROR: Medic tried to resurrect body from the body queue with no targetname!\n");
                return;
            }
        }
        ...
        if (!self->enemy->character)
        {
            gi.dprintf("ERROR: Medic failed to resurrect corpse of class '%s'!\n", self->enemy->classname);
...
void medic_hook_retract (edict_t *self)
{
    ...
    //SWB - we may not have an enemy if we tried to resurrect something that couldn't be resurrected
    if (self->enemy)
        self->enemy->monsterinfo.aiflags &= ~AI_RESURRECTING;
}
