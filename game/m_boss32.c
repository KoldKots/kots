...
void makron_torso_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
    ...
    //SWB - most corpses throw heads that they turn into, but the torso doesn't have a head
    //so we just make the torso invisible until it gets removed from the queue
...
void makron_torso (edict_t *ent)
{
    ...
    VectorSet (ent->mins, -60, -60, 0);
    VectorSet (ent->maxs, 60, 60, 72);
...
void makron_die (edict_t *self, edict_t *inflictor, edict_t *attacker, int damage, vec3_t point)
{
    ...
    //SWB - don't keep creating torsos
...
void SP_monster_makron (edict_t *self)
{
    ...
    self->health = 15000;
    self->gib_health = -990; //SWB - changed since anything under -999 can never be gibbed
    self->mass = 500;
...
void MakronSpawn (edict_t *self)
{
    ...
    //SWB - make makron invincible for 2 seconds
    if (self->character)
        self->character->next_candamage = level.time + 2;
}
