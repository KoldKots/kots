...
{
    //SWB - don't waste time ducking, fire more
    //if (random() > 0.25)
        return;
...
void GunnerGrenade (edict_t *self)
{
    ...
    if(self->enemy->client)
    {
        if(self->enemy->client->ps.pmove.pm_flags & PMF_DUCKED)
            target[2] -= 12; //Fire lower in the hope to hit them
    }
    ...
    monster_fire_grenade (self, start, aim, 50, 1000, flash_number);
}
